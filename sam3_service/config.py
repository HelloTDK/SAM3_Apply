...
# 仅在排查样例标注重框时输出最终 NMS 过程，默认关闭以避免生产日志膨胀。
NMS_DEBUG = _env_bool("SAM3_NMS_DEBUG", "0")
...
# 负样例二次检索过滤默认关闭，便于已有部署平滑升级；启用后按类别执行二次 grounding。
MULTI_NEGATIVE_FILTER_ENABLED = _env_bool("SAM3_MULTI_NEGATIVE_FILTER_ENABLED", "0")
MULTI_NEGATIVE_FILTER_CONFIDENCE = float(
    os.getenv("SAM3_MULTI_NEGATIVE_FILTER_CONFIDENCE", "0.9")
)
MULTI_NEGATIVE_FILTER_IOU = float(os.getenv("SAM3_MULTI_NEGATIVE_FILTER_IOU", "0.6"))
